...
// ─── S3 OPERATIONS ───────────────────────────────────────────────────────────
// S3-compatible via the object_store crate (same crate LanceDB uses internally).
// Supports AWS S3, Linode Object Storage, Cloudflare R2, MinIO — anything with
// an S3-compatible API.
//
// Credentials are ALWAYS read from environment:
//   AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY   ← required
//   PRZMA_S3_ENDPOINT, PRZMA_S3_BUCKET, PRZMA_S3_REGION  ← from StorageMode
//
// For BYOS mode, explicit short-lived credentials are passed via OptCreds.
// For CloudSaas, OptCreds is None and object_store reads from env.
...
fn build_s3_store(
    ...
    region:   &str,
    creds:    OptCreds<'_>,
) -> CalendarResult<object_store::aws::AmazonS3> {
    let mut b = AmazonS3Builder::new()
        .with_bucket_name(bucket)
        .with_region(region)
        .with_endpoint(endpoint);

    if let Some((access_key, secret_key, session_token)) = creds {
        b = b
    ...
    // No explicit creds → object_store reads AWS_ACCESS_KEY_ID +
    // AWS_SECRET_ACCESS_KEY from environment automatically.

    b.build().map_err(|e| CalendarError::Storage(e.to_string()))
}
...
async fn s3_read(
    ...
    let store = build_s3_store(endpoint, bucket, region, creds)?;
    let path  = OsPath::parse(key).map_err(|e| CalendarError::Storage(e.to_string()))?;
    let result = store.get(&path).await.map_err(|e| match e {
        object_store::Error::NotFound { .. } => CalendarError::NotFound(format!("s3:{}", key)),
        other                                => CalendarError::Storage(other.to_string()),
    })?;
    let bytes = result.bytes().await.map_err(|e| CalendarError::Storage(e.to_string()))?;
    Ok(bytes.to_vec())
}
...
async fn s3_write(
    ...
    let store = build_s3_store(endpoint, bucket, region, creds)?;
    let path  = OsPath::parse(key).map_err(|e| CalendarError::Storage(e.to_string()))?;
    store.put(&path, Bytes::from(data.to_vec()).into())
        .await
...
async fn s3_delete(
    ...
    let store = build_s3_store(endpoint, bucket, region, creds)?;
    let path  = OsPath::parse(key).map_err(|e| CalendarError::Storage(e.to_string()))?;
    store.delete(&path).await.map_err(|e| CalendarError::Storage(e.to_string()))?;
    Ok(())
}
