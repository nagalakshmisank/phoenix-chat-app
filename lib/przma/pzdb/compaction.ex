...
defmodule PRZMA.PzDb.Compaction do
  use GenServer
  require Logger
  ...
  def handle_info(:check_all, state) do
    ...
    active_dids = PRZMA.PzDb.VaultWriter.active_dids()
  ...
  defp needs_compaction?(did, service, space, table) do
    ...
    case PRZMA.Calendar.NIF.pzdb_version(base_path, table_path) do
      {:ok, version_json} ->
  ...
  defp run_compaction(did, service, space, table, state) do
    ...
    result = PRZMA.PzDb.VaultWriter.write(did, fn ->
      case PRZMA.Calendar.NIF.pzdb_compact(base_path, table_path) do
        {:ok, stats_json} ->
    ...
    case result do
      {:ok, _} ->
        ...
        PRZMA.PzDb.ReadCache.invalidate_table(did, service, space, table)
        %{state |
