...
defmodule PRZMA.PzDb.WriteRouter do
  ...
  def write(did, fun) when is_function(fun, 0) do
    ...
    if target == Node.self() do
      PRZMA.PzDb.VaultWriter.write(did, fun)
    else
  ...
  defp route_to_node(target_node, did, fun) do
    case :rpc.call(target_node, PRZMA.PzDb.VaultWriter, :write, [did, fun], 30_000) do
      {:badrpc, :nodedown} ->
        ...
        if fallback == Node.self() do
          ...
          :rpc.call(fallback, PRZMA.PzDb.VaultWriter, :write, [did, fun], 30_000)
          |> unwrap_rpc()
        end
