...
defmodule PRZMA.PzDbV2 do
  alias PRZMA.PzDbV2.{VaultWriter, WriteRouter, ReadCache, HealthMonitor}
  ...
  def batch_write(pzdb_table_uri, records, opts \\ [])
    ...
    with :ok <- HealthMonitor.check() do
      ...
      {dir, table} = resolve(pzdb_table_uri)
      key_cols     = opts[:key_columns] || ["id"]
  ...
  def delete(pzdb_uri, deleted_by) when is_binary(pzdb_uri) do
    with :ok <- HealthMonitor.check() do
      ...
      result = WriteRouter.write(did, fn ->
        NIF.pzdb_soft_delete(dir, table, record_id, deleted_by)
      end)
  ...
  def query(pzdb_table_uri, opts \\ []) when is_binary(pzdb_table_uri) do
    ...
    case NIF.pzdb_read_many(dir, table, filter, limit, min_version) do
      {:ok, json}   -> {:ok, Jason.decode!(json)}
      {:error, msg} -> {:error, msg}
    end
  end
  ...
  def version(pzdb_table_uri) when is_binary(pzdb_table_uri) do
    {dir, table} = resolve(pzdb_table_uri)
    case NIF.pzdb_version(dir, table) do
      {:ok, json}   -> {:ok, Jason.decode!(json)}
      {:error, msg} -> {:error, msg}
    end
  end
  ...
  ⚠️ NOT VERIFIED beyond the "files" entry below. Every path segment here was
  written for the old fused Uri resolver and is already the plural final
  table name — now that resolve/1 delegates to Namespace (which pluralizes
  automatically), most of these would double-pluralize ("entries" ->
  "entriess") and "social/..." isn't even in Namespace's @valid_services
  list yet. Out of scope for the file-service trial — fix per-service before
  calling this for anything beyond files.
  """
  ...
  defp do_read(pzdb_uri, did, min_version, decrypt) do
    ...
    case NIF.pzdb_read(dir, table, record_id, min_version) do
      {:ok, json} ->
  ...
  # Delegates to the same canonical Namespace resolver as production PzDb —
  # single source of truth for path shape, and this also fixes two bugs the
  # original enterprise Uri module had: it fused connection-dir + table-name
  # into one string (LanceDB rejects "/" in table names), and it never
  # sanitized ":" in DIDs before using them in a path.
  defp resolve(pzdb_uri) do
    PRZMA.Platform.Namespace.resolve(@base_path, pzdb_uri)
  end
