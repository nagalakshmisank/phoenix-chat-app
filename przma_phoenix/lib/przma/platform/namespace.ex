...
defmodule PRZMA.Platform.Namespace do
  @valid_services ~w(vault calendar chat files metadata ai agents creative companion auth social)
  ...
  # EXCEPTION: a few table names are matched *literally* by the Rust side's
  # schema_for/1 (cas_meta, auth, sessions, outbox, inbox). Pluralizing them
  # would silently fall through to the default files_schema() there — the
  # exact bug already fixed once for CAS ref_count. Never re-pluralize these.
  @exact_tables ~w(cas_meta auth sessions outbox inbox)
  defp table_for(res_type) when res_type in @exact_tables, do: res_type
